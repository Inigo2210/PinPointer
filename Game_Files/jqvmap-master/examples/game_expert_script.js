...
const countryList = {
	...
	function getCountry() {
		...
		highScore = localStorage.getItem("country_expert_high_score");
		if (highScore == null) {
	...
	function answerTest(ID) {
		...
		if (counter == 11) {
			...
			if ((points > parseInt(localStorage.getItem("country_expert_high_score"))) || (localStorage.getItem("country_expert_high_score") == null)) {
				var newRecord = points.toString(10);
				localStorage.setItem("country_expert_high_score", newRecord);
				document.getElementById("highScore").innerHTML = "Your Best Score: " + localStorage.getItem("country_expert_high_score"); 
			}
		}
		else {
